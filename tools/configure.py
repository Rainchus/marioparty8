...
ninja_file.variable('MAP', '$BUILD_DIR/mp8/MarioParty8.MAP')
...
ninja_file.rule('make_dol',
                command = "($ELF2DOL $in $out) && (sha1sum -c sha1/mp8.us.sha1)")

for s_file in s_files:
    ninja_file.build("build/" + append_extension(s_file), "s_files", s_file)

ninja_file.build("build/mp8.elf", "make_elf ", o_files)
ninja_file.build("build/mp8.dol", "make_dol ", "build/mp8.elf")
